...
function App() {
  ...
  const [softwareFormPrefillRecord, setSoftwareFormPrefillRecord] =
    useState<SoftwareInventoryRecord | null>(null);
      ...
      if (hasSoftwareName) {
        ...
        for (const [index, softwareName] of softwareNames.entries()) {
          ...
          const syncedSoftwareRecord: SoftwareInventoryRecord = {
            ...
            subscription: matchedRecord?.subscription || "",
            ...
            renewalDate: matchedRecord?.renewalDate || matchedRecord?.expiryDate || "",
            expirationDate: matchedRecord?.expirationDate || matchedRecord?.expiryDate || "",
  ...
  const handleSoftwareSubmit = async (record: SoftwareInventoryRecord) => {
    ...
    const normalizeSoftware = (value?: string) => String(value ?? "").trim().toLowerCase();
    const sameSourceKey =
      String(cleaned.sourceAccountabilityRecordId ?? "").trim() ||
      [cleaned.employeeId, cleaned.hostname].map((item) => String(item ?? "").trim().toLowerCase()).join("|");
    ...
    const hasDuplicateSoftware = softwareRecords.some((item) => {
      const itemSourceKey =
        String(item.sourceAccountabilityRecordId ?? "").trim() ||
        [item.employeeId, item.hostname].map((entry) => String(entry ?? "").trim().toLowerCase()).join("|");

      return (
        itemSourceKey === sameSourceKey &&
        normalizeSoftware(item.softwareName) === normalizeSoftware(softwareName)
      );
    });

    if (hasDuplicateSoftware) {
      window.alert(
        "Software already exists for this record. Use Edit on the existing entry instead of creating a new form."
      );
      setActiveView("records");
      return;
    }
    ...
    setSoftwareFormPrefillRecord(null);
    setSelectedModule("it-software-inventory");
    setActiveView("records");
  };

  const handleOpenNewSoftwareForm = () => {
    if (softwareRecords.length === 0) {
      window.alert(
        "Create at least one software record first. New software form is only for adding additional software."
      );
      setActiveView("records");
      return;
    }
    ...
    const baseRecord =
      ...
      editingSoftwareRecord ??
      [...softwareRecords].sort((left, right) =>
            ...
            {isSoftwareInventoryModule && (
              <>
                <button
                  type="button"
                  className={`nav-btn${activeView === "form" ? " nav-btn--active" : ""}`}
                  onClick={handleOpenNewSoftwareForm}
                >
                  <span className="nav-icon">✦</span>
                  Additional Software Form
                </button>
                <button
                  type="button"
                  className={`nav-btn${activeView === "records" ? " nav-btn--active" : ""}`}
        ...
        <main className="main-content">
          ...
          {isSoftwareInventoryModule && activeView === "form" && (
            <SoftwareInventoryForm
              ...
              prefillRecord={softwareFormPrefillRecord}
