...
const normalizeSoftwareRecord = (record: SoftwareInventoryRecord): SoftwareInventoryRecord => {
  const fallbackDate = String(record.expiryDate ?? "").trim();

  return {
    ...record,
    subscription: String(record.subscription ?? "").trim(),
    renewalDate: String(record.renewalDate ?? fallbackDate).trim(),
    expirationDate: String(record.expirationDate ?? fallbackDate).trim()
  };
};
...
export const useSoftwareInventoryRecords = () => {
  ...
  const loadRecords = useCallback(async () => {
    ...
    if (useLocalMode) {
      const local = readLocal().map(normalizeSoftwareRecord);
      setRecords(local);
      setLoading(false);
      return;
    }
  ...
  const createRecord = async (record: SoftwareInventoryRecord) => {
    const payload: SoftwareInventoryRecord = {
      ...normalizeSoftwareRecord(record),
      createdAt: stampNow(),
      updatedAt: stampNow()
    };
