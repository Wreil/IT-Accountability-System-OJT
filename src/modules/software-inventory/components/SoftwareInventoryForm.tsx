...
interface SoftwareInventoryFormProps {
  ...
  prefillRecord?: SoftwareInventoryRecord | null;
...
const labels: FieldDef[] = [
  ...
  { key: "vendor", label: "Vendor / Publisher" },
  {
    ...
    label: "Subscription",
    type: "select",
    options: ["", "Monthly", "Quarterly", "Annual", "Perpetual", "One-time", "Trial"]
  },
...
export const SoftwareInventoryForm = ({
  ...
  useEffect(() => {
    if (!editingRecord) {
      if (prefillRecord) {
        ...
        setForm({
          ...base,
          ...prefillRecord,
          renewalDate: prefillRecord.renewalDate ?? prefillRecord.expiryDate ?? "",
          expirationDate: prefillRecord.expirationDate ?? prefillRecord.expiryDate ?? "",
          id: undefined,
          createdAt: undefined,
          updatedAt: undefined,
          preparedSignature: prefillRecord.preparedSignature ?? base.preparedSignature,
          approvedSignature: prefillRecord.approvedSignature ?? base.approvedSignature
        });
      } else {
    ...
    setForm({
      ...base,
      ...editingRecord,
      renewalDate: editingRecord.renewalDate ?? editingRecord.expiryDate ?? "",
      expirationDate: editingRecord.expirationDate ?? editingRecord.expiryDate ?? "",
  ...
  return (
    <section className="panel" style={{ position: "relative" }}>
      <h2>
        {editingRecord
          ? "Edit Software Inventory Form"
          : prefillRecord
            ? "Create Additional Software Form"
            : "Create Software Inventory Form"}
      </h2>
      <button
        type="button"
        className="ghost"
      ...
      <p className="helper-text">
        {prefillRecord
          ? "Base employee/device details are prefilled. Enter the additional software details and save."
